...
void CMyApp::Update()
{
	...
	for (int i = 0; i < m_particleCount; ++i)
	{
		...
		m_particleVel[i].y -= 10 * delta_time;
...
void CMyApp::Render()
{
	...
	// kocka
	glPolygonMode(GL_BACK, GL_LINE);    //átlátunk a kockán
	m_vao.Bind();
	...
	{
		ImGui::SliderFloat("Energy remaining", &energyRemaining, 0, 2);
		...
		if (ImGui::Button("asd")) {
			...
			for (int i = 0; i < m_particleCount; ++i)
			{
				m_particlePos[i] = (glm::vec3(rnd(gen), rnd(gen), rnd(gen)));
				/*m_particleVel[i].y -= 10 * delta_time;*/
			}
