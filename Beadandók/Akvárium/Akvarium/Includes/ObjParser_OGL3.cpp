...
Mesh* ObjParser::parse(const char* fileName, bool quadMode)
{
	ObjParser theParser;
	...
	theParser.mesh = new Mesh(quadMode);

	while(theParser.skipCommentLine()) 
	{
		if (false == theParser.processLine(quadMode))
			break;
	}
...
bool ObjParser::processLine(bool quadMode)
{
	string line_id;
	float x, y, z;    
	...
	if ("v" == line_id) {	//	vertex data
		...
		for( unsigned int iFace = 0; iFace < (quadMode ? 4 : 3); iFace++ )
		{
			ifs >> iPosition;
			if( '/' == ifs.peek() )
			{
